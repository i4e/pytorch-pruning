...
class PrunningFineTuner_VGG16:
    ...
    def train_batch(self, optimizer, batch, label, rank_filters):
        ...
        if rank_filters:
            self.criterion(self.prunner.forward(input), Variable(label)).backward()
    ...
    def num_filters_each_layer(self):
        print("layer, # of filters")
        for name, module in self.model.features._modules.items():
            if isinstance(module, torch.nn.modules.conv.Conv2d):
                print(name, ",", module.out_channels)
                
    def prune(self, num_filters_to_prune_per_iteration, iterations):
...
def get_args():
    ...
    parser.add_argument("--pruned_filters_per_iter", type = int, default = 128)
    parser.add_argument("--pruning_iter", type = int, default = 3)
    parser.set_defaults(train=False)
    parser.set_defaults(prune=False)
    parser.set_defaults(test=False)
    parser.set_defaults(test_prune=False)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        fine_tuner.prune(args.pruned_filters_per_iter, args.pruning_iter)
        
    elif args.test:
        fine_tuner.num_filters_each_layer()
        print("total", fine_tuner.total_num_filters(), "filters")
